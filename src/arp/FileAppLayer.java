...
public class FileAppLayer implements BaseLayer {
    ...
    public boolean sortFileList(int lastFrameNumber) {
        ...
        if((fileByteList.size() - 1 != lastFrameNumber) || (receivedLength != targetLength)) {
            ((Dialog)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 실패\n");
            return false;
        }
    ...
    public void setAndStartSendFile() {
        Dialog upperLayer = (Dialog) this.GetUpperLayer(0);
        ...
        try (FileInputStream fileInputStream = new FileInputStream(sendFile)) {
            ...
            ((Dialog)this.GetUpperLayer(0)).progressBar.setMaximum(sendTotalLength);
            if(sendTotalLength <= 1448) {
                ...
                ((Dialog)this.GetUpperLayer(0)).progressBar.setValue(sendedLength);
            } else {
                sendedLength = 0;
                // 파일 정보 송신
    ...
    public synchronized boolean Receive(byte[] input) { // 데이터를 수신 처리 함수
        ...
        if(checkReceiveFileInfo(input)) { // 파일의 정보를 받은 경우
            ...
            // Progressbar 초기화
            ((Dialog)this.GetUpperLayer(0)).progressBar.setMinimum(0);
            ((Dialog)this.GetUpperLayer(0)).progressBar.setMaximum(targetLength);
            ((Dialog)this.GetUpperLayer(0)).progressBar.setValue(0);
            ...
            if (checkNoFragmentation(input)) {
                ...
                if(checkLastDataFrame(input)) {
                    ...
                    if(sortFileList(lastFrameNumber)) {
                        try(FileOutputStream fileOutputStream = new FileOutputStream(file)) {
                            ...
                            ((Dialog)this.GetUpperLayer(0)).ChattingArea.append("파일 수신 및 생성 완료\n");
                            fileByteList = new ArrayList();
                ...
                ((Dialog)this.GetUpperLayer(0)).progressBar.setValue(receivedLength); // Progressbar 갱신
            }
        }
        return true;
    }
